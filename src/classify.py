...
**별점은 분포가 퍼져야 의미가 있다.** 대부분의 논문은 2~3점이다.
4점은 상위 20%, 5점은 상위 2% 정도라고 생각하고 매겨라.
좋아 보인다고 전부 4점을 주면 정렬이 무의미해진다 —
실제로 그렇게 되어 하루치 게시분이 전부 동점이 된 적이 있다.
...
- **ehr_clinical과 medical_imaging은 '무엇을 입력으로 쓰는가'로 가른다.**
  진단·처방 코드, 검사 수치, 생체신호, 임상 노트처럼 **영상이 아닌 기록**을
  다루면 ehr_clinical이다. 영상(X-ray, CT, MRI, 병리, 내시경)이 주 입력이면
  medical_imaging이다. 둘을 함께 쓰면 주된 기여가 있는 쪽으로 보낸다.
  뇌 영상(fMRI/EEG)은 이 규칙보다 위의 brain_decoding 규칙이 우선한다.
