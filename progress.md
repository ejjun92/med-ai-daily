...
## 2. 현재 상태 (2026-09-03 16:00 기준)

**정상.** 2026-09-03자 49건 + Frontier 6건이 발행돼 있다. 다음 자동 실행은
내일 12:00 KST이며, 프롬프트 버전이 그대로라 **5~20분**이면 끝난다.

### 방금 지나온 사고 — 이틀 연속 실패 (09-02, 09-03)

arXiv가 **HTTP 429**(IP 단위 호출 제한)를 냈는데 `arxiv.py`만 예외를 밖으로
던져 파이프라인 전체가 죽었다. PubMed 244건·S2 3,000건은 멀쩡히 받아왔는데도
아무것도 발행하지 못했다. 09-01에 소속 기관 조사로 arXiv를 과하게 호출한 것이
발단이다. 사이트는 이틀 내내 09-01 내용에 멈춰 있었다.

**고친 것** (`test_source_failure.py` 4개가 지킨다):
- `ingest.collect`가 소스별 예외를 잡아 **그 소스만** 건너뛴다
- `IngestStats.failed`로 실패 소스를 기록하고 로그에 남긴다
- 429/503은 60초 기준 지수 백오프(총 7분). 기존 3초로는 IP 차단이 안 풀린다
- arXiv 성공 표시를 남기지 않으므로 다음 실행이 창을 늘려 공백을 메운다

**함께 발견한 것**: SSH 키 권한이 `0660`으로 풀려 git이 키를 무시했다.
파이프라인은 성공하는데 push만 조용히 실패하는 형태다. `chmod 600`으로 고쳤다.

09-02자 페이지는 따로 만들지 않았다. 30일 창이라 그날 논문이 09-03 수집분에
이미 들어 있어, 별도 발행은 같은 논문을 두 번 싣는 셈이 된다.
